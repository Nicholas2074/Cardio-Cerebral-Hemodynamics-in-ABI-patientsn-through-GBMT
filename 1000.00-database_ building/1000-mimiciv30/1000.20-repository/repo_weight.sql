...
wt_stg2 AS (
	SELECT wt_stg1.stay_id,
		...
		CASE
			...
			AND wt_stg1.rn = 1 THEN DATETIME_SUB(ie.intime, INTERVAL '2' HOUR)
			ELSE wt_stg1.charttime
		END AS starttime,
		wt_stg1.weight
	FROM wt_stg1
		INNER JOIN icu.icustays ie ON ie.stay_id = wt_stg1.stay_id
),
wt_stg3 AS (
	SELECT stay_id,
		intime,
...
wt_fix AS (
	...
	FROM icu.icustays ie
		INNER JOIN -- the below subquery returns one row for each unique stay_id

...
DROP TABLE IF EXISTS repo_ventilation;
CREATE TABLE repo_ventilation AS
...
WITH tm AS (
	SELECT stay_id,
		charttime
	FROM public.repo_ventilator_setting
	UNION
	DISTINCT
	SELECT stay_id,
		charttime
	FROM public.repo_oxygen_delivery
),
vs AS (
	SELECT tm.stay_id,
		tm.charttime, -- source data columns, here for debug
		o2_delivery_device_1,
		COALESCE(ventilator_mode, ventilator_mode_hamilton) AS vent_mode, -- case statement determining the type of intervention
		-- done in order of priority: trach > mech vent > NIV > high flow > o2
		CASE
			-- tracheostomy
			WHEN o2_delivery_device_1 IN (
				'Tracheostomy tube', -- 1135 observations for T-Piece
				-- could be either InvasiveVent or Tracheostomy, so omit
				-- 'T-piece',
				'Trach mask ' -- 16435 observations
			) THEN 'Tracheostomy' -- mechanical / invasive ventilation
			WHEN o2_delivery_device_1 IN (
				'Endotracheal tube'
			)
			...
			OR ventilator_mode_hamilton IN (
				...
				'NIV',
				'NIV-ST'
			) THEN 'NonInvasiveVent' -- high flow nasal cannula
			WHEN o2_delivery_device_1 IN (
				'High flow nasal cannula' -- 925 observations
			) THEN 'HFNC' -- non rebreather
			WHEN o2_delivery_device_1 IN (
			...
			WHEN o2_delivery_device_1 IN ('None') THEN 'None' -- not categorized: other
			ELSE NULL
		END AS ventilation_status
	FROM tm
		LEFT JOIN public.repo_ventilator_setting vs ON tm.stay_id = vs.stay_id
		AND tm.charttime = vs.charttime
		LEFT JOIN public.repo_oxygen_delivery od ON tm.stay_id = od.stay_id
...
vd0 AS (
	...
	FROM vs
	WHERE ventilation_status IS NOT NULL WINDOW w AS (
			PARTITION BY stay_id
			ORDER BY charttime
		)
),
vd1 AS (
	SELECT stay_id,
		...
		DATETIME_DIFF(charttime, charttime_lag, 'MINUTE') / 60 AS ventduration, -- now we determine if the current ventilation status is "new",
		-- or continuing the previous event
		CASE
			-- if lag is null, this is the first event for the patient
			WHEN ventilation_status_lag IS NULL THEN 1 -- a 14 hour gap always initiates a new event
			WHEN DATETIME_DIFF(charttime, charttime_lag, 'HOUR') >= 14 THEN 1 -- not a new event if identical to the last row
...
SELECT stay_id,
	...
	MAX(
		CASE
			...
			OR DATETIME_DIFF(charttime_lead, charttime, 'HOUR') >= 14 THEN charttime
			ELSE charttime_lead
		END
	) AS endtime,

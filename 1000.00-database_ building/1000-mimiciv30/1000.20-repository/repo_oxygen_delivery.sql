DROP TABLE IF EXISTS reop_oxygen_delivery;
CREATE TABLE repo_oxygen_delivery AS WITH ce_stg1 AS (
...
SELECT subject_id,
	...
	MAX(
		CASE
			WHEN itemid = 223834 THEN valuenum
			ELSE NULL
		END
	) AS o2_flow,
	MAX(
	...
	MAX(
		CASE
			WHEN rn = 2 THEN o2_device
			ELSE NULL
		END
	) AS o2_delivery_device_2,
	MAX(
		CASE
			WHEN rn = 3 THEN o2_device
			ELSE NULL
		END
	) AS o2_delivery_device_3,
	MAX(
		CASE
			WHEN rn = 4 THEN o2_device
			ELSE NULL
		END

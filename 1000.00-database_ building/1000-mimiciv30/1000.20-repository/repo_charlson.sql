	...
	SELECT pt.hadm_id,
		...
		MAX(
			CASE
				...
				OR SUBSTR(icd10_code, 1, 4) = 'H340' THEN 1
				ELSE 0
			END
		...
		MAX(
			CASE
				...
				OR SUBSTR(icd10_code, 1, 4) IN ('F051', 'G311') THEN 1
				ELSE 0
			END
		) AS dementia,
		...
		MAX(
			CASE
				...
				OR SUBSTR(icd10_code, 1, 4) = 'I252' THEN 1
				ELSE 0
			END
		...
		MAX(
			CASE
				...
				OR SUBSTR(icd10_code, 1, 4) IN ('I278', 'I279', 'J684', 'J701', 'J703') THEN 1
				ELSE 0
			END
		...
		MAX(
			CASE
				...
				OR SUBSTR(icd10_code, 1, 4) IN ('M315', 'M351', 'M353', 'M360') THEN 1
				ELSE 0
			END
		...
		MAX(
			CASE
				...
				OR SUBSTR(icd10_code, 1, 3) IN ('K25', 'K26', 'K27', 'K28') THEN 1
				ELSE 0
			END
		...
		MAX(
			CASE
				...
				OR SUBSTR(icd10_code, 1, 3) BETWEEN 'C90' AND 'C97' THEN 1
				ELSE 0
			END
		) AS malignant_cancer,
		MAX(
			CASE
				...
				OR SUBSTR(icd10_code, 1, 3) IN ('C77', 'C78', 'C79', 'C80') THEN 1
				ELSE 0
			END
		...
		MAX(
			CASE
				...
				OR SUBSTR(icd10_code, 1, 3) IN ('B20', 'B21', 'B22', 'B24') THEN 1
				ELSE 0
			END
		) AS aids
...
vw2 AS (
	SELECT hadm_id,
		CASE
			...
			ELSE 4
		END AS age
	FROM public.repo_age
)
-- //ANCHOR - total
SELECT pt.subject_id,
	pt.hadm_id,
	vw1.cerebrovascular_disease,
	vw1.paraplegia,
	vw1.dementia,
	vw1.myocardial_infarct,
	vw1.congestive_heart_failure,
	vw1.peripheral_vascular_disease,
	vw1.chronic_pulmonary_disease,
	vw1.rheumatic_disease,
	vw1.peptic_ulcer_disease,
	vw1.mild_liver_disease,
	vw1.severe_liver_disease,
	vw1.renal_disease,
	GREATEST(2 * vw1.diabetes_with_cc, vw1.diabetes_without_cc) AS diabetes,
	-- vw1.diabetes_with_cc AS diabetes_with_cc,
	-- vw1.diabetes_without_cc AS diabetes_without_cc,
	vw1.malignant_cancer,
	vw1.metastatic_solid_tumor,
	vw1.aids,
	vw2.age,
	(vw1.cerebrovascular_disease 
	+ vw1.paraplegia 
	...
	+ vw1.rheumatic_disease 
	+ vw1.peptic_ulcer_disease 
	+ GREATEST(vw1.mild_liver_disease, 3 * vw1.severe_liver_disease) 
	+ vw1.renal_disease 
	+ GREATEST(2 * vw1.diabetes_with_cc, vw1.diabetes_without_cc) 
	+ GREATEST(2 * vw1.malignant_cancer, 6 * vw1.metastatic_solid_tumor) 
	+ vw1.aids
	+ vw2.age) AS charlson
FROM hosp.admissions pt
	LEFT JOIN vw1 ON pt.hadm_id = vw1.hadm_id
	LEFT JOIN vw2 ON vw1.hadm_id = vw2.hadm_id;

DROP TABLE IF EXISTS repo_icustay_detail;
CREATE TABLE repo_icustay_detail AS
SELECT ie.subject_id,
	...
	ROUND(
		CAST(
			DATETIME_DIFF(ie.outtime, ie.intime, 'HOUR') / 24.0 AS NUMERIC
		),
		2
	) AS los_icu,
	DENSE_RANK() OVER (
...
FROM icu.icustays ie
	INNER JOIN hosp.admissions adm ON ie.hadm_id = adm.hadm_id
	INNER JOIN hosp.patients pat ON ie.subject_id = pat.subject_id;